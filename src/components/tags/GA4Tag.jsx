...
function InfoBox({ children, color = "blue", className = "" }) {
  const styles = {
    ...
    yellow:
      "bg-yellow-50 dark:bg-yellow-900/20 border-yellow-200 dark:border-yellow-800/40 text-yellow-800 dark:text-yellow-300",
  };
  return (
    <div
      className={`border rounded p-3 text-xs mb-4 leading-relaxed ${styles[color] ?? styles.blue} ${className}`}
    >
      {children}
    </div>
  );
...
function Helper({ children }) {
  return (
    <p className="text-xs text-gray-400 dark:text-zinc-500 mt-1">{children}</p>
  );
}

/* TAB 1 — CONFIGURATION */
function ConfigurationTab() {
  const [title, setTitle] = useState("Google Analytics 4");
  const [notes, setNotes] = useState("");
  const [pixelId, setPixelId] = useState("");
  const [genEventId, setGenEventId] = useState("True");
  ...
  const inputClass =
    "border border-gray-300 dark:border-zinc-600 rounded px-2 py-1 text-xs bg-white dark:bg-zinc-800 dark:text-zinc-200 focus:outline-none focus:ring-1 focus:ring-blue-400";
    ...
    <div className="flex">
      <div className="flex-1 px-6 py-5">
        ...
        <Field label="Measurement ID">
          <input value={pixelId} className={`w-64 ${inputClass}`} />
          <Helper>
            Measurement ID of the Google Analytics property to which you want to
            send data (e.g. G-12345678M).
          </Helper>
        </Field>
        <Field label="Global Object">
          <input value="" className={`w-64 ${inputClass}`} />
          <Helper>
            This is the name of the Global Object used for the event queue, if
            not specified "gtag" is used. This is not required for most
            implementations.
          </Helper>
        </Field>
        <Field label="Data Layer Name">
          <input value="" className={`w-64 ${inputClass}`} />
          <Helper>
            By default, the data layer initiated and referenced by the global
            site tag is named dataLayer. Rename the data layer only if your
            project requires a separate name.
          </Helper>
        </Field>
        <Field label="Send Page View">
          <SelectField
            value={genEventId}
          ...
          />
          <Helper>
            Automatically generate an Event ID for every Snap tracking event.
          </Helper>
        </Field>
        ...
        <Field label="Tag Timing">
          ...
          />
          <Helper>
            Determines the timing of the tag firing. Pre-load tags fire
            immediately on page loads. DOM Ready tags wait to fire until the
            page HTML has been completely loaded by the browser (the default).
          </Helper>
        </Field>
        ...
        <Field label="Custom Script Source">
          ...
          <Helper>
            ...
            from an alternate location. If you are using an absolute URL, it is
  ...
  return (
    <div className="flex flex-col">
      <div className="mx-4 mt-4">
        <InfoBox color="red">
          No debemos de modificar nada en esta sección para asegurarnos de que
        ...
        <div className="flex-1 border-r border-gray-200 dark:border-zinc-700 px-6 py-5">
          ...
          <div className="flex items-start gap-2 bg-blue-50 dark:bg-zinc-800 border border-blue-100 dark:border-zinc-700 rounded p-3 mb-5">
            ...
            <p className="text-xs text-blue-700 dark:text-blue-300">
              By default, the tag will fire on all pages and when any configured
              event occurs.
            </p>
          </div>

          <DropZone
            label="Drag a rule here"
            id="include"
            hoverDrop={hoverDrop}
            setHoverDrop={setHoverDrop}
          />
          <p className="text-xs font-semibold text-gray-500 dark:text-zinc-400 my-4">
            AND NOT
          </p>
          <DropZone
            label="Drag a rule here"
...
const VARS_DATA = [
  ...
  },
  {
    ...
    variable: "order_id",
    type: "UDO Variable",
    destination: "purchase.transaction_id",
    destColor: "blue",
  },
  {
    id: 3,
    variable: "order_currency",
    type: "UDO Variable",
    destination: "purchase.currency",
    destColor: "blue",
  },
  {
    id: 4,
  ...
  },
  {
    ...
    type: "UDO Variable",
    destination: "order_confirmation_view.purchase",
    destColor: "orange",
  },
  {
    id: 6,
  ...
  },
  {
    ...
    type: "UDO Variable",
    destination: "purchase.item.name",
    destColor: "blue",
  },
];
...
const DEST_COLOR_MAP = {
  blue: {
    line: "bg-blue-400",
    badge: "bg-blue-100 dark:bg-zinc-800 text-gray-700 dark:text-blue-300",
  },
  orange: {
    line: "bg-blue-400",
    badge:
      "bg-orange-200 dark:bg-orange-900/30 text-gray-700 dark:text-orange-300",
  },
};
  ...
  return (
    <div className="px-6 py-5">
      <InfoBox color="red">
        ...
        , dirigirse al apartado <strong>Event-specific parameters</strong> e
        introducir el nombre de la variable exactamente como figura en la
        ...
        , seguir la ruta: <strong>Add mapping → Events</strong>
      </InfoBox>
              ...
              return (
                ...
                >
                  <td className="px-4 py-2.5">
                    <div className="flex items-center gap-2">
                      <span className="text-red-400 cursor-move select-none text-sm">
                        ≡
                      </span>
                      <div>
                  ...
                  <td className="px-4 py-2.5">
                    <div className="flex items-center gap-2">
                      ...
                      <span
                        className={`${colors.badge} px-2 py-0.5 rounded text-xs font-medium whitespace-nowrap`}
                      >
                        {v.destination}
                      </span>
                    </div>
